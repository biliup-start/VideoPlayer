...
def check_biliup():
    ...
    if sys.platform == 'win32':
        if not os.access("./biliup/biliup.exe", os.F_OK):
            input("Biliup未正确安装, 回车自动安装:")

            import requests
            r = requests.get('https://github.com/biliup/biliup/releases/download/v1.1.28/biliupR-v1.1.28-x86_64-windows.zip', stream=True)
            ...
            # 文件归位
            shutil.move(f'.temp/{file_dir}/biliup.exe', './biliup/biliup.exe')
        ...
        if not os.path.exists("biliup/biliup"):
